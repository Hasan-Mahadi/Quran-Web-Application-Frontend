...
//         className="bg-white dark:bg-gray-800 rounded-lg shadow-md hover:shadow-xl transition-all duration-300 cursor-pointer p-6 border border-gray-200 dark:border-gray-700"
//       >
...
export default function SurahCard({ surah, index }: SurahCardProps) {
  const getRevelationIcon = (type: string) => {
    return type === 'Meccan' ? <Star className="w-3 h-3" /> : <Clock className="w-3 h-3" />;
  };
